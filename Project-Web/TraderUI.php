<?php
    ...
    if (!isset($_SESSION['Trader_id'])) {
        ...
        //select only products in selected shop 
            ...
            $trd_prod_qry = ' SELECT Product_id, Product_name, Product_image, Description, Allergy_information, Initial_price, stock_quantity  FROM product p  WHERE p.shop_id ='.$_GET['shop_id'];
             $trd_parse = oci_parse($conn, $trd_prod_qry);
        }
            ...
            $shop_qry = ' SELECT Shop_id,Shop_name FROM shop WHERE Trader_id = :trader_id';
            ...
            /*$d_qry = 'SELECT discount_name FROM Product p ,discount d WHERE d.discount_id = p.discount_id';
...
<body>
    ...
    <div class="container-fluid pt-3 trader_UI_container">
        <div class="row">
            <div style="text-align: center; color: #6c8a5e;" >
                 <?php
                if (isset($_GET['errors'])) {
                    echo '<h4>'.$_GET['errors'].'</h4>';
                }
                elseif(isset($_GET['add'])){
                    echo '<h4>'.$_GET['add'].'</h4>';   
                }
                elseif (isset($_GET['update'])) {
                    echo '<h4>'.$_GET['update'].'</h4>';
                }
                ?>
            </div>
                    ...
                    <div class="dropdown shop_dropdown">
                        ...
                        <ul class="dropdown-menu shop_list" aria-labelledby="dropdownMenuButton1">
                            <?php
                                ...
                                //fetching shops
                            ?>
                            <li><a class="dropdown-item" href="TraderUI.php?shop_name=<?php echo $shops['SHOP_NAME']?>&shop_id=<?php echo $shops['SHOP_ID']?>"><?php echo $shops['SHOP_NAME']  ?></a></li>
                            <?php
                ...
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        <form action="php/upload_product.php" method="POST" enctype="multipart/form-data">
                            <input type="hidden" name="shop_id" value="<?php if(isset($_GET['shop_id'])){ echo $_GET['shop_id']; } ?>">
                            <div class="row">
                                <div class="col-md-5">
                            ...
                            <div class="row">
                             ...
                             <div class="row">
                                ...
                                <div class="col-md-7">
                                    ...
                                    ?>">
                                    <?php
                                        $qry = 'SELECT discount_name FROM Discount';
                                        $parse = oci_parse($conn, $qry);
                                        oci_execute($parse);
                                        while ($dis = oci_fetch_assoc($parse)) {
                                        //fetch all discount available
                                    ?>
                                        <option value="<?php echo $dis['DISCOUNT_NAME']?>"><?php echo $dis['DISCOUNT_NAME']?></option>
                                    <?php
                                        }
                                    ?>
                                    </select>
